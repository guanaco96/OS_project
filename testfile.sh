...
./client -l $1 -k pluto -S "Ti mando un file":pippo -s ./chatty:pippo -s ./chatty.c:pippo
if [[ $? != 0 ]]; then
    exit 1
fi
...
if [[ $md5lib != $md53 ]]; then
    echo "./chatty.c e $2/chatty.c differiscono!"
    exit 1
fi
